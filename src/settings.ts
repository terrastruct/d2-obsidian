...
export class D2SettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc("Customize the local path to the directory `d2` is installed in. This is only necessary if `d2` is not found automatically by the plugin (but is installed).")
			.addText((text) => {
				text
					.setPlaceholder("/usr/local/Cellar")
          .setValue(this.plugin.settings.d2Path).onChange(
					async (value) => {
